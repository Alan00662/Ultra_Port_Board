...
uint16_t lcd_framebuf[320][480] = {0}; // ֱ�����ڲ�RAM
...
void MX_LTDC_Init(void)
{
    ...
    g_ltdc_framebuf[0] = (uint16_t *)lcd_framebuf;
    ...
    //  pLayerCfg.FBStartAdress =  (uint32_t)g_ltdc_framebuf[0];
    ...
    //    HAL_LTDC_SetWindowSize(&hltdc, 320, 480, 0);
...
void ltdc_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint32_t color)
{
...
#if 1
    addr = ((uint32_t)g_ltdc_framebuf[0] + 2 * (320 * sy + sx));
    offline = 480 - (ex - sx + 1); // ��ƫ��
#else
    addr = ((uint32_t)g_ltdc_framebuf[0] + 2 * (320 * sy + sx));
    offline = 320 - (ex - sx + 1);
#endif
    ...
    DMA2D->CR = DMA2D_R2M;          // �Ĵ������ڴ�
    DMA2D->OPFCCR = LTDC_PIXEL_FORMAT_RGB565;
    DMA2D->OOR = offline;
    DMA2D->OMAR = addr;
    DMA2D->NLR = (ey - sy + 1) | ((ex - sx + 1) << 16); // ������������16λ������
    //    DMA2D->NLR = (ex - sx + 1) | ((ey - sy + 1) << 16); //������������16λ������
    DMA2D->OCOLR = color;
